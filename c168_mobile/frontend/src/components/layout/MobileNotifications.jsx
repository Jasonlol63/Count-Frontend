...
import { fetchDashboardAnnouncements, normalizeAnnouncementItem } from "../../lib/announcementApi.js";
import "./mobile-notifications.css";

export async function fetchMobileAnnouncements(signal) {
  const { res, json } = await fetchDashboardAnnouncements({ signal });
  if (!res.ok || !json?.success) return [];
  return (Array.isArray(json.data) ? json.data : []).map(normalizeAnnouncementItem);
}
