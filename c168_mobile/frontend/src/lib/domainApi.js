 ...
 *
 * No endpoint here depends on the session's "current tenant" at all (confirmed against
 * DomainController.java — none of its methods touch SecurityUtils/session), so unlike
 * other modules there is no `switchSessionTenant`/session-sync step anywhere in this file.
 */
import { buildApiUrl } from "../utils/apiUrl.js";
import { fetchAccountListByTenantId, normalizeAccountListItem } from "./accountApi.js";
import {
...
function tenantRowFromAggregate(type, t, parentCode = null) {
  ...
  const feeShareAllocations = feeShareSpringToUi(t.feeShareAllocations ?? t.fee_share_allocations);
...
/**
 * Global tenant code uniqueness — pure client-side check against the already-loaded
 * `domains` list (no network call; replaces the old `validate_domain_code` PHP action).
...
export function validateTenantCodeGlobally(code, { excludeOwnerId, domains = [] } = {}) {
  ...
  if (!want) return { ok: false, message: "codeRequired" };
  if (want === "C168") return { ok: false, message: "cannotRenameToC168" };

  for (const owner of domains) {
    if (excludeOwnerId != null && Number(owner.id) === Number(excludeOwnerId)) continue;
...
/** POST /api/domain/list?ownerId= (query param despite POST — matches Spring's @RequestParam). */
...
export async function createDomain({ ownerCode, name, email, password, secondaryPassword, groups, companies }) {
...
export async function updateDomain({ id, ownerCode, name, email, password, secondaryPassword, groups, companies }) {
  const body = {
    id,
    owner_code: ownerCode,
    name,
...
export async function updateTenantSetting({
  ...
  const body = { id, code, ownerId, expirationDate: expirationDate || null };
  if (permissions != null) body.featureModules = permissionNamesToFeatureModules(permissions);
  if (feeShareAllocations != null) body.feeShareAllocations = feeShareUiToSpring(feeShareAllocations, id);
...
/** Merge tenant ids from Spring DomainDTO response (camelCase Tenant list) back into local rows. */
...
/** Persist feature modules + fee share for every tenant (post create/update skeleton). */
...
function findC168OwnerCompanyRow(companies) {
  const rows = companies || [];
  return rows.find(
    (c) => String(c.tenant_code ?? c.company_id ?? c.code ?? "").trim().toUpperCase() === "C168",
  );
}

/** Session tenant field readers — same fallback-to-legacy pattern as desktop's sessionTenant.js. */
function getSessionTenantId(me) {
...
export function resolveShareLedgerTenantId(me, companies = null) {
  ...
  if (sessionId && (isCurrentTenantC168(me) || sessionCode === "C168")) return sessionId;
...
export function resolveShareLedgerTenantCode(me, companies = null) {
  ...
  const c168Row = findC168OwnerCompanyRow(companies);
  const fromList = String(c168Row?.tenant_code ?? c168Row?.company_id ?? c168Row?.code ?? "").trim().toUpperCase();
...
export async function fetchShareAccountsForTenant(tenantId, { signal } = {}) {
  ...
  const active = rows.map(normalizeAccountListItem).filter((a) => a && String(a.status || "").toLowerCase() === "active");
  ...
  return { accounts, accounts_profit: accountsProfit };
}
