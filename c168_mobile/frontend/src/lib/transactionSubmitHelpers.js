...
export function buildRatePayload({
  ...
  const payload = {
    ...
    /* ------------------------------------------------------------------
     * Spring 专用字段组（`buildSpringSubmitRequest` 只读这一组）。
     * 命名带 `leg1_`/`leg2_` 前缀，跟上面那批 legacy `rate_*` 字段刻意区分开，
     * 因为两套字段的语义不总是一一对应——尤其 leg2：
     *   - legacy 的 `rate_transfer_from_account_id` 存的是 **UI To Account**（命名与 UI 交叉）；
     *   - 这里的 `leg2_to_account_id` 直接用 UI 语义，所以取值同样来自 `transferToId`。
     * 另外**不发 `leg2_amount`**：后端 2026-08 起 leg2 恒记毛额、根本不读前端这个字段，
     * 旧的 "Leg2 amount must equal…" 校验也已删除（见后端文档 §4.7）。
     * ------------------------------------------------------------------ */
    leg1_to_account_id: toId,
    leg1_from_account_id: fromId,
    leg1_currency: rateCurrencyFrom,
    leg1_amount: storeFrom,
    // FX 原始文本（如 "/1.5"）——不传它，Rate-Mul 的 divide 模式与"点数模式"会在后端
    // 静默退化成 0（不报错，只是中间人佣金消失）。
    rate_expression: String(rateExchangeRateRaw ?? "").trim(),
    middleman_rate_expression: rateMiddlemanRate ? String(rateMiddlemanRate).replace(/,/g, "").trim() : "",
    // Fee 面值：只认 `rateMiddlemanInputAmount`（原始 Fee 输入）。绝不能拿
    // `rateMiddlemanAmount`（中间人总利润 = Rate-Mul 佣金 + 净 Fee）兜底，否则只填
    // Rate-Mul 的提交会被误标成一笔 Fee。
    middleman_fee_amount: rateMiddlemanInputAmount ? cleanAmt(rateMiddlemanInputAmount) : "",
    middleman_platform_fee_amount: rateMiddlemanPlatformFee ? cleanAmt(rateMiddlemanPlatformFee) : "",
  };
  ...
  if (transferToId && transferFromId) {
    ...
    /* Spring leg2 组。注意上面两行 legacy 字段是**交叉**的（`rate_transfer_from_*` 存 UI To），
       而这两个按 UI 语义直取，别照抄上面的赋值方向。leg2 现在是必填（Spring 侧
       `leg2_transaction_id` 是必填外键），所以这一整块必须执行到。 */
    payload.leg2_to_account_id = transferToId; // UI "To Account"
    payload.leg2_from_account_id = transferFromId; // UI "From Account"
