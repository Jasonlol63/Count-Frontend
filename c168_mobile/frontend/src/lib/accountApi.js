/**
 * Account + Currency — Spring Boot `/api/account/*` + `/api/currency/*`.
 * Copied from Count-frontend/src/pages/account/accountListApi.js +
 * src/utils/api/currencyApi.js (desktop) — same backend, same contract, so the
 * request/response shapes here are kept byte-for-byte identical to those files.
 *
 * Note: unlike the old PHP `accountlistapi.php` (which took `company_id` / `group_id` /
 * `group_only` query params and did the group-ledger aggregation server-side), the Spring
 * endpoint only takes a single `tenant_id` — a GROUP is just another tenant row now, so
 * "list this group's own accounts" and "list this company's accounts" are the exact same
 * call with a different tenant_id. Group-wide / groups-all aggregation is done client-side
 * by calling this once per tenant id and merging (see `fetchMergedAccountLists`).
 */
...
function normalizeAccountTenantIds(tenantIds) {
  return normalizeAccountCurrencyIds(tenantIds);
}

/** Spring UserListDTO → page row (snake_case, same shape the mobile UI already used). */
...
export async function fetchMergedAccountLists(
  ...
  const slices = await Promise.all(ids.map((tid) => fetchAccountListByTenantId(tid, signal).catch(() => [])));
  const byId = new Map();
...
/** Flip payment_alert via POST /api/account/update (preserves currencyIds/tenantIds). */
export async function toggleAccountUserPaymentAlert(row, scopeTenantId, currencyIds, signal) {
  ...
  const tenantIds = normalizeAccountTenantIds(row.tenant_ids).length ? row.tenant_ids : [scopeTenantId];
...
export async function fetchAccountLinkedAccounts(accountId, tenantId, signal) {
  ...
  const params = new URLSearchParams({ account_id: String(aid), tenant_id: String(tid) });
  const res = await fetch(buildApiUrl(`api/account/link/list?${params}`), {
    credentials: "include",
    signal,
  });
  const json = await res.json();
  if (!res.ok || !json.success) throw new Error(json?.message || "failedToLoadAccountLinks");
  ...
  return { accounts, linkTypesMap, tenantId: Number(data.tenant_id ?? data.tenantId ?? tid) };
}

/** POST /api/account/link */
export async function linkAccountPair({ accountId1, accountId2, linkType = "bidirectional", sourceAccountId = null }, signal) {
  const a1 = Number(accountId1);
  const a2 = Number(accountId2);
  if (!Number.isFinite(a1) || a1 <= 0 || !Number.isFinite(a2) || a2 <= 0) {
    throw new Error("invalidRequest");
  }
  const body = { accountId1: a1, accountId2: a2, linkType: uiLinkTypeToSpring(linkType) };
...
export async function unlinkAccountPair({ accountId1, accountId2, tenantId }, signal) {
  ...
  const params = new URLSearchParams({ account_id_1: String(a1), account_id_2: String(a2), tenant_id: String(tid) });
...
export async function updateAccountLinkPair({ accountId1, accountId2, linkType = "bidirectional", sourceAccountId = null }, signal) {
  const a1 = Number(accountId1);
  const a2 = Number(accountId2);
  if (!Number.isFinite(a1) || a1 <= 0 || !Number.isFinite(a2) || a2 <= 0) {
    throw new Error("invalidRequest");
    ...
    raw?.is_linked != null
      ...
      : raw?.isLinked != null
        ...
        : raw?.linked != null
          ? !!raw.linked
          : !!isLinked;
  const deletable = raw?.deletable != null ? !!raw.deletable : syncSource !== "subsidiary";
  return {
    id: Number(raw?.id),
...
/**
 * POST /api/currency/list?tenant_id= — a tenant's full Currency Setting list.
 *
 * The right source for a currency *picker*. Deliberately NOT `fetchAvailableCurrencies`
 * (`/api/currency/available`): that endpoint's `is_linked` flag is only meaningful when a concrete
 * `account_id` is passed, and without one the backend always answers `false` — using it as a picker
 * empties the list entirely. The backend's dashboard record calls that mistake out explicitly
 * (`Count/docs/dashboard-springboot-kpi.md` §3).
 */
export async function fetchCurrencyListByTenantId(tenantId, signal) {
  const tid = resolveAccountListTenantId(tenantId);
  if (!tid) throw new Error("tenantIdRequired");
  const res = await fetch(buildApiUrl(`api/currency/list?tenant_id=${encodeURIComponent(tid)}`), {
    method: "POST",
    credentials: "include",
    signal,
  });
  const json = await res.json();
  if (!res.ok || !json.success) throw new Error(json?.message || "failedToLoadCurrencies");
...
export async function createTenantCurrency({ code, tenantId }, signal) {
  ...
  const res = await fetch(buildApiUrl("api/currency/add"), {
    ...
    headers: { "Content-Type": "application/json" },
    credentials: "include",
    body: JSON.stringify({ tenantId: String(tid), code: String(code || "").trim().toUpperCase() }),
    signal,
  });
  const json = await res.json();
...
export async function updateAccountsLinkedToCurrency({ tenantId, currencyId, linkedAccountIds = [], unlinkedAccountIds = [] }, signal) {
  const tid = resolveAccountListTenantId(tenantId);
  const cid = Number(currencyId);
  if (!tid || !Number.isFinite(cid) || cid <= 0) throw new Error("invalidRequest");
