...
export function transactionScopeApiParams(scope) {
  ...
  if (scope.mode === "aggregate") {
    return {
      ...
      // Spring is single-tenant per request — the API layer loops over these tenant ids
      // and merges, rather than the old PHP server doing the aggregation itself.
      mergeCompanyIds: scope.mergeCompanyIds?.length ? scope.mergeCompanyIds : undefined,
    };
  }
