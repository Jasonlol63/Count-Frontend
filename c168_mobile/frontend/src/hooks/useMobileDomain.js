...
export function useMobileDomain() {
  ...
  const loadDomains = useCallback(
    async ({ silent = false } = {}) => {
      try {
        ...
        if (!silent) setError(e?.message || t("failedToLoadDomainData"));
        return false;
      }
    },
    [refreshFeeSummary, t],
  ...
  const executeBulkDelete = useCallback(
    async (validOwners) => {
      ...
      try {
        const results = await Promise.all(valid.map((d) => deleteOwner(d.id).then(({ json }) => json)));
  ...
  return {
    ...
    isOwnerOrAdmin,
    isDeletable,
    companyCode: resolveShareLedgerTenantCode(me, companies),
    groupId: "",
    shareLedgerTenantId: resolveShareLedgerTenantId(me, companies),
  };
}
