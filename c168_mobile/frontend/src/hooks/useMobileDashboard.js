...
import { fetchCurrentUser, logoutSession, switchSessionTenantWithReason } from "../lib/authApi.js";
...
import { buildSpringTrendChartRows, resolveDailyChartXAxisTicks } from "../lib/dashboardChart.js";
...
import { buildKpiFromSpringPayload } from "../lib/dashboardKpi.js";
import {
...
import { breakdownToPanelRows, loadMobileDashboardData } from "../lib/dashboardSpringLoad.js";
import {
...
/**
 * Currency / Earning tab rows, from the Spring currency-breakdown.
 *
 * Kept the old `earningsRowsFromBootstrap` guarantee on purpose: the **primary** currency's row is
 * pinned to the painted KPI card's numbers so the pie/list can never disagree with the hero cards.
 * Everything else (native amount, converted amount, rate, earnings, converted earnings) now comes
 * straight from the server instead of being recomputed here.
 */
function panelRowsFromBreakdown(
  bootstrap,
  { panelNetProfit = null, panelEarnings = null, primaryCurrency } = {},
) {
  const primary = String(primaryCurrency || "").toUpperCase();
  const rows = breakdownToPanelRows(bootstrap?.breakdown);
  if (rows.length) {
    return rows.map((row) => ({
      ...row,
      netProfit: row.code === primary && panelNetProfit != null ? panelNetProfit : row.netProfit,
      earnings: row.code === primary && panelEarnings != null ? panelEarnings : row.earnings,
    }));
  }

  // No breakdown rows came back for this scope → fall back to a single row built from the KPI.
  const code = String(bootstrap?.currency || primaryCurrency || "MYR").toUpperCase();
  const netProfit = panelNetProfit ?? bootstrap?.kpi?.netProfit ?? null;
  const earnings = panelEarnings ?? bootstrap?.kpi?.earnings ?? null;
  if (netProfit == null && earnings == null) return [];
  return [{ code, netProfit, netProfitConverted: null, earnings, earningsConverted: null, rate: null }];
}
...
export function useMobileDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // The `import.meta.env.DEV` demo-payload substitution was dropped here: `DEMO_BOOTSTRAP`
        // modelled the legacy bootstrap feed (`period_total` / `daily_data` / per-currency
        // payloads), which nothing reads any more, so swapping it in only produced an empty
        // dashboard. The new shape's presence check is `bootstrap.kpi` (see `hasData` below).
  ...
  /**
   * KPI cards, straight off the server DTO.
   *
   * Gone: `resolveMobileKpiOwnershipOpts` + two `computeKpiMetrics` calls. Those emulated the
   * scope and re-derived the ownership-weighted Earnings figure on the client from fields the
   * Spring response doesn't have. `buildKpiFromSpringPayload` now maps the finished numbers
   * (`showEarnings` / `earnings` / the whole `previous*` set) onto the card shape, so the
   * previous-period comparison comes from the same payload instead of a second bootstrap fetch.
   */
  const kpi = useMemo(() => buildKpiFromSpringPayload(bootstrap?.kpi), [bootstrap]);

  /** Backend aligns the previous range by −1 month and reports it; label from those bounds. */
  const compareLabel = useMemo(() => {
    const from = bootstrap?.previousDateFrom;
    const to = bootstrap?.previousDateTo;
    if (from && to) {
      return `${i18n.vsPreviousPeriod} (${formatRangeLabel(from, to, { withYear: false })})`;
    }
    const map = {
  ...
  const chartRows = useMemo(
    () => buildSpringTrendChartRows(bootstrap?.trend, dateFrom, dateTo),
  ...
  const earningsCurrencyRows = useMemo(() => {
    // Native and converted figures both come from `/api/dashboard/*/currency-breakdown` now
    // (`originalAmount` / `amount` / `earnings` / `earningsConverted`), so the client-side
    // Frankfurter re-conversion that used to run when `useConvertedEarnings` was on is gone — the
    // server already converted each row, and `rate` is null rather than 0 when it has no quote.
    return panelRowsFromBreakdown(bootstrap, {
      panelNetProfit: panelMetric,
      panelEarnings: panelEarningsMetric,
      primaryCurrency: currency,
    });
  }, [bootstrap, panelMetric, panelEarningsMetric, currency]);
  ...
  const syncCompanySession = useCallback(
    async (id, signal) => {
      const { ok, json, accessReason } = await switchSessionTenantWithReason(id, { signal });
      if (!ok || !json?.success) {
        if (accessReason) {
          const modalMessage =
            accessReason === "no_set" ? i18n.companyAccessNoSet : i18n.companyAccessExpired;
