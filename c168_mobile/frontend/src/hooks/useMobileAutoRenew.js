...
import { canAccessC168AutoRenew, fetchOwnerCompaniesForDomain } from "../lib/c168DomainAccess.js";
...
export function useMobileAutoRenew() {
  ...
  // AutoRenewController's endpoints take no tenant/company param at all (confirmed
  // against the Java source) — no session-company-sync step needed here any more.
  ...
  const approve = useCallback(
    async ({ requestId, period, chargeOnApprove = true }) => {
      setBusyId(requestId);
      try {
        await approveAutoRenew({ requestId, period, chargeOnApprove });
