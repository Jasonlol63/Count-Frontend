...
import { fetchCurrentUser, loginWithTenant } from "../../lib/authApi.js";
import { consumeMaintenanceKickNotice } from "../../lib/maintenanceNotice.js";
...
export default function LoginPage() {
  ...
  /* IT turned the "kick everyone" maintenance switch on: `MobileShell` forced this session here
     and left a flag. Explain why, the same way a login blocked by maintenance mode is explained
     below. Consumed once so a manual reload doesn't re-show it. */
  useEffect(() => {
    if (consumeMaintenanceKickNotice()) {
      showNotice(i18n.maintenanceModalTitle);
    }
  }, [showNotice, i18n.maintenanceModalTitle]);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (data.status === "success" && data.redirect) {
        ...
        let me = null;
        try {
          const { ok: meOk, json: meJson } = await fetchCurrentUser();
      ...
      // System maintenance mode (IT kick-switch) blocked this login — credentials were
      // correct, but the backend refused. Same signal shape as desktop's LoginPage.jsx.
      if (data?.data?.maintenanceMode) {
        showNotice(localizeAuthApiMessage(data.message, lang) || i18n.maintenanceModalTitle);
        return;
      }
