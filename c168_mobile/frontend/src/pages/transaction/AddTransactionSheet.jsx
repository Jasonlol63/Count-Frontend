...
export default function AddTransactionSheet({
  ...
  const handleSubmit = async () => {
    ...
    if (isRate) {
      ...
      /* 第二组账户（Transfer）现在是必填：Spring 的 `transactions_rate.leg2_transaction_id` 是必填
         外键，一定要有一笔真实的第二币种 Cr/Dr。legacy PHP 时代"不填就只有第一币种那一笔账"的规则
         不适用于 Spring 提交路径（见 Count-frontend/docs/transaction-rate-springboot-submit.md §1）。 */
      ...
      if (!transferToId || !transferFromId) {
        return pushToast(m.pleaseSelectRateTransferAccounts, "error");
      }
