...
export const LOGIN_I18N = {
  en: {
    ...
    loginBackendOffline: "Cannot reach the backend server. Please try again shortly.",
    loginServerError: "Server error (HTTP {status}). Please try again.",
    loginInvalidResponse: "Server returned an invalid response.",
    confirm: "Confirm",
  ...
  zh: {
    ...
    loginFailed: "登录失败",
    loginError: "登录时发生错误",
    loginBackendOffline: "无法连接服务器，请稍后重试。",
    loginServerError: "服务器错误 (HTTP {status})，请稍后重试。",
    loginInvalidResponse: "服务器返回异常。",
    confirm: "确认",
...
/**
 * Reset Password (TAC flow) — `POST /auth/send-reset-tac` + `POST /auth/reset-password`.
 * Copied from Count-frontend/src/translateFile/auth/authTranslate.js (desktop
 * `RESET_PASSWORD_I18N`); mobile's field/flow wording is identical.
 *
 * Note `tacSent` is deliberately the neutral "if this account exists…" phrasing: the backend never
 * reveals whether an account exists (see `AuthController#sendResetTac`), so the UI must not imply
 * otherwise.
 */
export const RESET_PASSWORD_I18N = {
  en: {
    ...
    tacSent: "If this account exists, a verification code has been sent.",
  ...
  zh: {
    ...
    success: "成功",
    confirm: "确定",
    companyIdFirst: "请先输入公司 ID",
    emailFirst: "请先输入邮箱地址",
    invalidEmailFormat: "请输入有效的邮箱地址",
    tacSent: "如果该账号存在，验证码已发送。",
    tacFailed: "验证码发送失败，请重试。",
    networkError: "网络错误，请重试。",
    passwordsNoMatch: "两次输入的密码不一致",
    enterTac: "请输入验证码",
    companyEmailRequired: "公司 ID 与邮箱为必填项",
