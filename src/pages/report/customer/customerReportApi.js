 ...
 * the Spring backend only ever answers for one tenant at a time (see resolveCustomerReportTenantId).
 */
export async function fetchCustomerReport(
  ...
  const tenantIds = mergeCompanyIds.length > 0
    ? mergeCompanyIds.map((id) => Number(id))
    : [resolveCustomerReportTenantId(reportScope)];
...
export async function fetchAccounts(reportScope, options = {}) {
  ...
  if (!tenantIds.length || tenantIds.some((id) => !id)) return [];
